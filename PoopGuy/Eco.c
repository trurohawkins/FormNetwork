...
void eVap(int x, int y){
	...
	//block is uncovered
		...
		if (theWorld->map[x][y - depth].count == 0) {
			return;
		}
...
// if block is at bottom of world, water drops outs into the ether
	...
	if (theWorld->map[x][y-1].count == 0) { return; }
